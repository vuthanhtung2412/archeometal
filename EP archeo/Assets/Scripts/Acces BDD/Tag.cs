...
public class Tag
{
    ...
    public static bool AssociateObjectToTag(int idObj, int idTag)
    {
        ...
        Console.WriteLine($"INSERT INTO tag_map (id_tag, id_obj) VALUES ({idTag},{idObj});");
        return true;
    }
    ...
    public static Tag? CreateNewTag(string tagName, string parentName)
    {
        ...
        var idTagInDb = int.Parse(Database
            .DataReader($"SELECT id_tag FROM tag WHERE tag_name = '{tagName}' AND parent_tag = '{parentName}';").Rows[0]
            .ItemArray
            .Select(x => x.ToString()).ToArray()[0]);
    ...
    public static bool ImportCsvToDataBase(string pathToFile)
    {
        ...
        // Set parser options
        using var parser = new TextFieldParser(pathToFile);
    ...
    private static void InsertListToDatabase(List<string[]> stringsList)
    {
        ...
        // Delete table header
        var firstItemOfList = stringsList.First();
        var header1 = firstItemOfList.First();
        if (header1.Equals("Catégorie fonctionnelle"))
            stringsList.Remove(firstItemOfList);
    ...
    public static List<int> GetObjectsAssociatedWithTag(int idTag)
    {
        ...
        return (from DataRow row in queryResult.Rows select int.Parse(row["id_obj"].ToString()!)).ToList();
    }
}